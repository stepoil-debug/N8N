...
IGNORED_EMPTY_SHEET_TITLES = {"roteiro de fabricacao"}
...
def pdf_content(data: bytes) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    if not any(page["text"] for page in pages):
        ...
        try:
            ...
            pages = [
                {"page": index, "text": pytesseract.image_to_string(image, lang="por+eng").strip()}
                for index, image in enumerate(convert_from_bytes(data, dpi=200), 1)
            ]
...
def spreadsheet_rows(ws) -> list[list[str]]:
    rows: list[list[str]] = []
    for row in ws.iter_rows(values_only=True):
        normalized = ["" if value is None else str(value) for value in row]
        if any(value not in (None, "") and str(value).strip() for value in row):
            rows.append(normalized)
    return rows


def should_ignore_blank_sheet(sheet_name: str, rows: list[list[str]]) -> bool:
    return fold(sheet_name) in IGNORED_EMPTY_SHEET_TITLES and not rows


def extract(filename: str, data: bytes) -> dict[str, Any]:
    suffix = Path(filename).suffix.casefold()
    warnings: list[str] = []
    ignored_sheets: list[dict[str, str]] = []
    if suffix == ".pdf":
        ...
        for ws in book.worksheets:
            rows = spreadsheet_rows(ws)
            if should_ignore_blank_sheet(ws.title, rows):
                ignored_sheets.append(
                    {
                        "sheet": ws.title,
                        "reason": "Aba de ROTEIRO DE FABRICAÇÃO em branco por padrão; ignorada nas validações e notificações.",
                    }
                )
                continue
            content.append({"sheet": ws.title, "rows": rows})
        kind = "spreadsheet"
    elif suffix == ".docx":
    ...
    if ignored_sheets:
        result["ignored_sheets"] = ignored_sheets
    return result
...
async def prepare(request: Request) -> dict[str, Any]:
    ...
    if not oid:
        raise HTTPException(status_code=422, detail="opportunity_id é obrigatório")
    meta = {
        "opportunity_id": oid,
...
async def extract_document(file: UploadFile = File(...), opportunity_id: str = Form(default="unassigned")) -> dict[str, Any]:
    ...
    if len(data) > MAX_UPLOAD:
        raise HTTPException(status_code=413, detail="Arquivo acima do limite configurado")
...
async def checklist(request: Request) -> dict[str, Any]:
    ...
    if not oid or not isinstance(analysis, dict):
        raise HTTPException(status_code=422, detail="opportunity_id e analysis são obrigatórios")
    requirements = analysis.get("requirements") or []
    if not isinstance(requirements, list):
        raise HTTPException(status_code=422, detail="requirements deve ser lista")
    ...
    ws.title = "Checklist"
    headers = [
        "ID", "Categoria", "Requisito", "Origem / Documento", "Localização da evidência",
        "Evidência resumida", "Tipo sugerido pela IA", "Prioridade sugerida", "Tratamento sugerido",
        "Decisão do usuário", "Tipo final", "Prioridade final", "Tratamento final", "Responsável final",
    ]
    ws.append(headers)
    ...
    for index, item in enumerate(requirements, 1):
        ws.append([
            item.get("id") or f"RFQ-{index:03d}", item.get("category", ""), item.get("requirement", ""),
            item.get("source_document", ""), item.get("source_location", ""), item.get("source_evidence", ""),
            item.get("type_ai", ""), item.get("priority_ai", ""), item.get("treatment_ai", ""),
            item.get("decision", "A confirmar"), item.get("type_final", ""), item.get("priority_final", ""),
            item.get("treatment_final", ""), item.get("owner_final", ""),
        ])
    ...
    summary.append(["Cliente", (analysis.get("summary") or {}).get("client", "")])
    summary.append(["RFQ", (analysis.get("summary") or {}).get("rfq_id", "")])
    summary.append(["Total de requisitos", len(requirements)])
    wb.save(output)
    ...
    def col(*names: str) -> int | None:
        for name in names:
            if name.casefold() in headers:
                return headers[name.casefold()]
        return None

    idc, reqc = col("ID"), col("Requisito")
    if not idc or not reqc:
        raise HTTPException(status_code=422, detail="Checklist sem colunas ID/Requisito")
    rows = []
    for row in range(2, ws.max_row + 1):
        rid = str(ws.cell(row, idc).value or "").strip()
        requirement = str(ws.cell(row, reqc).value or "").strip()
        if not rid and not requirement:
            continue
        ...
        rows.append({
            "checklist_id": rid or f"ROW-{row:04d}",
            "source_sheet": ws.title,
            "source_row": row,
            "requirement": requirement,
            "category": str(ws.cell(row, col("Categoria") or 1).value or "").strip(),
            "requirement_source": str(ws.cell(row, col("Origem / Documento") or 1).value or "").strip(),
            "requirement_location": str(ws.cell(row, col("Localização da evidência") or 1).value or "").strip(),
            "requirement_evidence": str(ws.cell(row, col("Evidência resumida") or 1).value or "").strip(),
            "applicable": decision.casefold() != "excluir" and final_type.casefold() != "não aplicável",
        })
    return {
...
async def inventory(file: UploadFile = File(...), opportunity_id: str = Form(default="unassigned")) -> dict[str, Any]:
    ...
    try:
        return inventory_from_file(source) | {"opportunity_id": opportunity_id}
    finally:
...
async def inventory_artifact(request: Request) -> dict[str, Any]:
    body = await request.json()
    oid = str(body.get("opportunity_id", "")).strip()
    name = safe(str(body.get("artifact_name", "")))
...
async def report(request: Request) -> dict[str, Any]:
    ...
    if (unmapped or no_points) and not body.get("allow_incomplete"):
        raise HTTPException(status_code=422, detail={
            "message": "Cobertura incompleta",
            "unmapped": unmapped,
            "items_without_comparison_points": no_points,
        })
    scores = {"Atendido": 1.0, "Parcial": 0.5, "Não atendido": 0.0}
    weights = {"Crítico": 4, "Alto": 3, "Médio": 2, "Baixo": 1, "Informativo": 0}
    num = den = 0.0
    blockers = []
    rows = [["ID", "Critério", "Pedido", "Proposta", "Status"]]
    for item in items:
        for point in item.get("comparison_points") or []:
            assessment = point.get("human_decision") or point.get("ai_assessment") or {}
            status = assessment.get("status", "Não verificável")
            impact = assessment.get("impact", "Médio")
            weight = float(weights.get(impact, 0))
            if status in scores and weight > 0:
                num += scores[status] * weight
                den += weight
            if assessment.get("blocks") and status in {"Não atendido", "Parcial"}:
                blockers.append(point.get("id"))
            rows.append([point.get("id", ""), point.get("parameter", ""), point.get("requested", ""), point.get("proposed", ""), status])
    adherence = round(num / den * 100, 1) if den else None
    coverage = round(len(inventory_ids & mapped_ids) / len(inventory_ids) * 100, 1) if inventory_ids else None
    decision = (
        "ANÁLISE INCOMPLETA" if unmapped or no_points else
        "NÃO ACEITÁVEL" if blockers else
        "ACEITÁVEL" if adherence is not None and adherence >= 95 else
        "ACEITÁVEL COM RESSALVAS" if adherence is not None and adherence >= 85 else
        "NÃO ACEITÁVEL"
    )
    ...
    story = [
        Paragraph("VALIDAÇÃO DE ADERÊNCIA DA PROPOSTA", styles["Title"]),
        Spacer(1, 5 * mm),
        Paragraph(f"Decisão: <b>{decision}</b>", styles["Heading2"]),
        Paragraph(
            f"Aderência: {adherence if adherence is not None else '-'}% | Cobertura: {coverage if coverage is not None else '-'}%",
            styles["BodyText"],
        ),
        Spacer(1, 5 * mm),
    ]
    table = Table(rows, repeatRows=1, colWidths=[25 * mm, 45 * mm, 42 * mm, 42 * mm, 30 * mm])
    ...
    story.append(table)
    doc.build(story)
    result = artifact(oid, output)
    result.update({
        "decision": decision,
        "adherence_percent": adherence,
        "coverage_percent": coverage,
        "blocking_points": blockers,
    })
    return result
