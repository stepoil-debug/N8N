...
from .drawing_vision import is_drawing_candidate, prepare_drawing_visuals
...
SUPPORTED = {".pdf", ".xlsx", ".xlsm", ".docx", ".csv", ".txt", ".md", ".json", ".eml", ".msg"} | IMAGE_EXTENSIONS
GROUPS = (
...
def classify_type(path: str, group: str) -> str:
    ...
    if ext in IMAGE_EXTENSIONS:
        return "drawing"
    if "lista de materiais" in name or "material list" in name or "bom" in name:
        return "material_list"
    if group == "proposal" or "proposta" in name or "proposal" in name:
        return "proposal"
    if group == "estimate" or "orcament" in name or "estimate" in name:
        return "estimate"
    if group == "material_quotes" or "cotacao" in name or "quotation" in name:
        return "material_quote"
    if group == "purchase_order":
        return "purchase_order"
    if ext == ".pdf" and any(token in name for token in ("sob-", "str-", "dwg", "drawing", "desenho", "croqui", "iso-", "isometric", "spool", "weld map", "mapa de solda", "support", "suporte")):
        return "drawing"
    if group == "rfq":
        return "rfq_document"
    if group == "clarifications":
...
def infer_metadata(filename: str, root: str | None) -> dict[str, str | None]:
    ...
    if not match:
        match = re.search(r"(\d{2})\s*[-_. ]\s*(\d{3})\s*bep", text)
    ...
    if client_match and client_match.group(1) not in {"enc", "rev"}:
        client = client_match.group(1).upper()
    if not client and "perenco" in text:
        client = "PERENCO"
...
def preview(extracted: dict[str, Any]) -> dict[str, Any]:
    ...
    elif extracted.get("kind") == "email" and content:
        text = f"{content[0].get('subject', '')}\n{content[0].get('body', '')}"
    elif extracted.get("kind") == "document" and content:
...
def _image_extraction(name: str, raw: bytes, extension: str) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "kind": "image",
            "content": [],
            "warnings": [f"Não foi possível ler metadados da imagem: {exc}"],
        }
...
def analyze_package(filename: str, data: bytes, opportunity_id: str | None = None, *, include_content: bool = True) -> dict[str, Any]:
    if len(data) > MAX_PACKAGE:
        raise HTTPException(status_code=413, detail="ZIP acima do limite configurado")
    if not zipfile.is_zipfile(io.BytesIO(data)):
        raise HTTPException(status_code=415, detail="O arquivo enviado não é um ZIP válido")
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as archive:
        ...
        if len(infos) > MAX_ENTRIES:
            raise HTTPException(status_code=413, detail="ZIP contém entradas demais")
        if sum(item.file_size for item in infos) > MAX_UNCOMPRESSED:
            raise HTTPException(status_code=413, detail="Conteúdo descompactado excede o limite")

        preliminary_root = None
        for info in infos:
            candidate = normalize_path(info.filename)
            if candidate:
                preliminary_root = PurePosixPath(candidate).parts[0]
                break
        inferred_early = infer_metadata(filename, preliminary_root)
        resolved_early = opportunity_id or inferred_early.get("opportunity_id") or safe(Path(filename).stem, "opportunity")
        ...
        for info in infos:
            ...
            if info.file_size > MAX_ENTRY:
                raise HTTPException(status_code=413, detail=f"Arquivo interno excede o limite: {path}")
            ...
            if name.casefold() in IGNORED or name.startswith("~$"):
                ignored.append({"path": path, "reason": "system_file", "size_bytes": info.file_size})
                continue

            group, owner = classify_group(path)
            document_type = classify_type(path, group)
            item: dict[str, Any] = {
                "path": path,
                "filename": name,
                "extension": ext,
            ...
            if ext in SUPPORTED:
                try:
                    ...
                    extracted = _image_extraction(name, raw, ext) if ext in IMAGE_EXTENSIONS else extract(name, raw)
                    ...
                    if raw is not None and is_drawing_candidate(path, document_type):
                        visuals = prepare_drawing_visuals(name, raw, str(resolved_early))
                        item["drawing_visuals"] = visuals
                        item["warnings"].extend(visuals.get("warnings") or [])
                except Exception as exc:  # noqa: BLE001
    ...
    summary = {
        ...
        "groups": dict(Counter(x["group"] for x in entries)),
        "source_owners": dict(Counter(x["source_owner"] for x in entries)),
        "document_types": dict(Counter(x["document_type"] for x in entries)),
        "extraction": dict(Counter(x["extraction_status"] for x in entries)),
        "drawings_prepared": sum(1 for x in entries if (x.get("drawing_visuals") or {}).get("status") == "prepared"),
        "drawing_pages_prepared": sum(len((x.get("drawing_visuals") or {}).get("pages") or []) for x in entries),
    }
    result: dict[str, Any] = {
        "status": "classified",
